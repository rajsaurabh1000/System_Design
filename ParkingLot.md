...
|                                                              |
|  ┌────────────┐  ┌────────────┐  ┌────────────┐  ┌────────┐ |
|  | Entry Gate |  | Exit Gate  |  | Display    |  | Payment| |
|  | (Camera,   |  | (Camera,   |  | Boards     |  | Kiosk  | |
|  | RFID, LPR) |  | Barrier)   |  | (LED/LCD)  |  | (POS)  | |
|  └──────┬─────┘  └──────┬─────┘  └──────┬─────┘  └────┬───┘ |
+─────────┼────────────────┼──────────────┼────────────┼────+
          │                │              │            │
          ▼                ▼              ▼            ▼
...
|              (Local / On-Prem Processing Unit)               |
|                                                              |
|  • Handles gate logic                                         |
|  • Works offline                                             |
|  • Talks to sensors & barriers                               |
...
|         │                   │                   │              |
|  ┌──────▼────────┐   ┌──────▼────────┐   ┌──────▼────────┐     |
|  | Spot Assignment |   | Analytics     |   | Notification  |     |
|  | Service         |   | Service       |   | Service       |     |
|  └─────────────────┘   └────────────────┘   └────────────────┘ |
+───────────────────────────┬──────────────────────────────────+
                            │
...
|  | PostgreSQL        |   | Redis          |                  |
|  | (Tickets, Spots,  |   | (Cache +       |                  |
|  |  Payments, Config)|   |  Availability) |                  |
|  └──────────────────┘   └────────────────┘                  |
|                                                              |
|  ┌──────────────────┐   ┌────────────────┐                  |
|  | TimescaleDB      |   | Kafka          |                  |
|  | (Metrics, Stats) |   | (Event Stream) |                  |
|  └──────────────────┘   └────────────────┘                  |
+──────────────────────────────────────────────────────────────+
...
<img width="1577" height="1350" alt="hLTHSzis37xNh-1RdJNExLcNvPh5pSvtNUTpqhup4gpmGf4QIILpk_xt0HH5Kx9at-xg9n7u008VG93-O1qthgrbPb-4QhZXDIiaj_RMEIEUMWVhGgi7yGtOhmb42WLktm3JnZthB7l7VflxeUvE6s0hNNEXs1qv1tlI8lCHi0qtk67rIJlsFMDioQHswp9dNxwiLtwjU0qvinYeghnW9xKsDcUVX7NNmVoUP3TU3SgP_H_S" src="https://github.com/user-attachments/assets/9d80aca2-1470-4aeb-a268-d640b17ce878" />
