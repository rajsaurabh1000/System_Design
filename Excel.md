...
**Algorithm**:
```python
def recalculate_dependents(changed_cell):
    # Get all cells that depend on changed_cell
    dependents = dependency_graph.get_dependents(changed_cell)
    
    # Topological sort to get correct calculation order
    sorted_cells = topological_sort(dependents)
    
    # Recalculate in order
    for cell in sorted_cells:
        cell.calculate()
        
        # If result changed, add its dependents
        if cell.result_changed():
            dependents.extend(
                dependency_graph.get_dependents(cell)
            )
```
...
public class CellService {
    ...
    public CellUpdateResult updateCell(
        UUID sheetId,
        String cellAddress,
        Object value,
        String formula,
        ...
        // 1. Get sheet (from cache or DB)
        Sheet sheet = getSheet(sheetId);
        Cell cell = sheet.getCell(cellAddress);
        
        Object oldValue = cell != null ? cell.getValue() : null;
        
        // 2. Create or update cell
        if (cell == null) {
            cell = new Cell(cellAddress);
            sheet.addCell(cell);
        }
        
        // 3. Handle formula if present
        Set<String> dependencies = new HashSet<>();
        Object calculatedValue = value;
        
        if (formula != null && !formula.isEmpty()) {
            // Parse and validate formula
            Formula parsedFormula = formulaEngine.parseFormula(formula);
            if (parsedFormula.hasError()) {
                throw new InvalidFormulaException(parsedFormula.getError());
            }
            
            // Get dependencies
            dependencies = parsedFormula.getDependencies();
            
            // Update dependency graph
            dependencyGraph.updateDependencies(cellAddress, dependencies);
            
            // Check for circular references
            if (dependencyGraph.detectCycle(cellAddress)) {
                throw new CircularReferenceException(cellAddress);
            }
            
            // Evaluate formula
            Map<String, Object> context = getCellValues(sheet, dependencies);
            calculatedValue = formulaEngine.evaluateFormula(parsedFormula, context);
            
            cell.setFormula(parsedFormula);
        } else {
            // Remove from dependency graph if was previously a formula
            dependencyGraph.removeDependencies(cellAddress);
        }
        
        // 4. Set cell value
        cell.setValue(calculatedValue);
        cell.setUpdatedAt(Instant.now());
        cell.setUpdatedBy(userId);
        
        // 5. Persist to database
        sheetRepository.saveCell(sheetId, cell);
        
        // 6. Invalidate cache
        cacheManager.invalidateCell(sheetId, cellAddress);
        
        // 7. Find and recalculate dependent cells
        Set<String> affectedCells = recalculateDependents(sheet, cellAddress);
        affectedCells.add(cellAddress);
        
        // 8. Create operation for collaboration
        ...
        // 9. Broadcast to collaborators
        collaborationManager.broadcastOperation(operation);
        
        // 10. Return result with all affected cells
        Map<String, Object> affectedCellsData = new HashMap<>();
        for (String addr : affectedCells) {
            Cell c = sheet.getCell(addr);
            affectedCellsData.put(addr, c.getValue());
        }
    ...
    private Set<String> recalculateDependents(Sheet sheet, String changedCell) {
        // Get all cells that depend on this cell
        Set<String> dependents = dependencyGraph.getAllDependents(changedCell);
        ...
        List<String> calculationOrder = 
            dependencyGraph.getTopologicalOrder(dependents);
        
        Set<String> recalculated = new HashSet<>();
        
        for (String cellAddress : calculationOrder) {
            Cell cell = sheet.getCell(cellAddress);
            if (cell == null || cell.getFormula() == null) {
                continue;
            }
            
            // Get context values for formula
            Set<String> deps = cell.getFormula().getDependencies();
            Map<String, Object> context = getCellValues(sheet, deps);
            
            // Recalculate
            Object newValue = formulaEngine.evaluateFormula(
                cell.getFormula(),
                context
            );
            
            // Only update if value changed
            if (!Objects.equals(cell.getValue(), newValue)) {
                cell.setValue(newValue);
                cell.setUpdatedAt(Instant.now());
                sheetRepository.saveCell(sheet.getId(), cell);
                recalculated.add(cellAddress);
            }
        }
    ...
    private Map<String, Object> getCellValues(Sheet sheet, Set<String> addresses) {
        Map<String, Object> values = new HashMap<>();
        for (String address : addresses) {
            Cell cell = sheet.getCell(address);
            values.put(address, cell != null ? cell.getValue() : null);
        }
        return values;
    }
}
...
public class FormulaEngine {
    ...
    public Formula parseFormula(String expression) {
        try {
            ...
            Set<String> dependencies = extractDependencies(ast);
            
            return new Formula(expression, ast, dependencies);
        } catch (ParseException e) {
            return Formula.error(e.getMessage());
        }
    }
    
    public Object evaluateFormula(Formula formula, Map<String, Object> context) {
        try {
            return evaluate(formula.getAst(), context);
        } catch (Exception e) {
            return "#ERROR: " + e.getMessage();
        }
    }
    
    private Object evaluate(ASTNode node, Map<String, Object> context) {
        switch (node.getType()) {
            case NUMBER:
            ...
            case CELL_REFERENCE:
                String address = (String) node.getValue();
                return context.getOrDefault(address, null);
                
            case RANGE:
                String range = (String) node.getValue();
                return expandRange(range, context);
            ...
            case FUNCTION_CALL:
                String funcName = (String) node.getValue();
    ...
    private Object callFunction(String name, List<Object> args) {
        FormulaFunction function = functions.get(name.toUpperCase());
        if (function == null) {
    ...
    private List<Object> expandRange(String range, Map<String, Object> context) {
        ...
        for (int row = start.getRow(); row <= end.getRow(); row++) {
            for (int col = start.getColumn(); col <= end.getColumn(); col++) {
                String addr = CellAddress.toString(row, col);
                values.add(context.getOrDefault(addr, null));
            }
        }
    ...
    private Set<String> extractDependencies(ASTNode node) {
        Set<String> deps = new HashSet<>();
        
        if (node.getType() == NodeType.CELL_REFERENCE) {
            deps.add((String) node.getValue());
        } else if (node.getType() == NodeType.RANGE) {
            String range = (String) node.getValue();
            deps.addAll(expandRangeAddresses(range));
        }
        
        // Recursively process child nodes
        if (node.getLeft() != null) {
            deps.addAll(extractDependencies(node.getLeft()));
        }
        if (node.getRight() != null) {
            deps.addAll(extractDependencies(node.getRight()));
        }
        if (node.getArguments() != null) {
            for (ASTNode arg : node.getArguments()) {
                deps.addAll(extractDependencies(arg));
            }
        }
...
public class DependencyGraph {
    // Map: cell -> cells that depend on it
    private final Map<String, Set<String>> dependents;
    
    // Map: cell -> cells it depends on
    ...
    public void updateDependencies(String cell, Set<String> newDeps) {
        ...
        Set<String> oldDeps = dependencies.get(cell);
        if (oldDeps != null) {
            for (String dep : oldDeps) {
                dependents.computeIfPresent(dep, (k, v) -> {
                    v.remove(cell);
        ...
        if (!newDeps.isEmpty()) {
            dependencies.put(cell, new HashSet<>(newDeps));
    ...
    public void removeDependencies(String cell) {
        updateDependencies(cell, Collections.emptySet());
    }
    ...
    public Set<String> getAllDependents(String cell) {
        ...
        while (!queue.isEmpty()) {
            ...
            Set<String> deps = getDependents(current);
            for (String dep : deps) {
    ...
    public List<String> getTopologicalOrder(Set<String> cells) {
        ...
        for (String cell : cells) {
            if (!visited.contains(cell)) {
                topologicalSortUtil(cell, visited, visiting, result);
            }
        }
    ...
    private void topologicalSortUtil(
        ...
        Set<String> deps = dependencies.getOrDefault(cell, Collections.emptySet());
        for (String dep : deps) {
            ...
            if (!visited.contains(dep)) {
                topologicalSortUtil(dep, visited, visiting, result);
            }
        }
    ...
    public boolean detectCycle(String startCell) {
        Set<String> visited = new HashSet<>();
        Set<String> recStack = new HashSet<>();
        return detectCycleUtil(startCell, visited, recStack);
    }
    ...
    private boolean detectCycleUtil(
        ...
        for (String dep : deps) {
            if (!visited.contains(dep)) {
                if (detectCycleUtil(dep, visited, recStack)) {
                    return true;
                }
...
#### 7.2.4 Collaboration Manager (WebSocket)

```java
@Component
public class CollaborationManager {
    private final Map<UUID, Map<UUID, WebSocketSession>> workbookSessions;
    private final OTEngine otEngine;
    private final CellService cellService;
    
    public CollaborationManager() {
        this.workbookSessions = new ConcurrentHashMap<>();
    ...
    public void handleConnection(
        ...
        workbookSessions
            .computeIfAbsent(workbookId, k -> new ConcurrentHashMap<>())
            .put(userId, session);
        
        // Notify others about new user
        broadcastUserJoined(workbookId, userId);
    }
    
    public void handleDisconnection(UUID workbookId, UUID userId) {
        Map<UUID, WebSocketSession> sessions = workbookSessions.get(workbookId);
        if (sessions != null) {
            sessions.remove(userId);
            if (sessions.isEmpty()) {
                workbookSessions.remove(workbookId);
            }
        }
        
        // Notify others about user leaving
        broadcastUserLeft(workbookId, userId);
    }
    ...
    public void handleOperation(
        ...
        try {
            ...
            Operation transformed = otEngine.transform(operation);
            
            // Apply operation (update cell)
            cellService.applyOperation(transformed);
            
            // Broadcast to all other users
            broadcastOperation(workbookId, userId, transformed);
            
            // Send acknowledgment to originating user
            sendAck(workbookId, userId, operation.getId(), transformed.getVersion());
            
        } catch (Exception e) {
            sendError(workbookId, userId, operation.getId(), e.getMessage());
        }
    }
    ...
    public void handleCursorMove(
        ...
        CursorMessage message = new CursorMessage(userId, cellAddress);
        broadcast(workbookId, message, userId);
    }
    ...
    private void broadcastOperation(
        ...
        broadcast(workbookId, message, originUserId);
    }
    ...
    private void sendAck(
        ...
        WebSocketSession session = getSession(workbookId, userId);
        if (session != null) {
            AckMessage ack = new AckMessage(operationId, serverVersion);
            send(session, ack);
        }
    }
    ...
    private void sendError(
        ...
        WebSocketSession session = getSession(workbookId, userId);
        if (session != null) {
            ErrorMessage msg = new ErrorMessage(operationId, error);
            send(session, msg);
        }
    }
    
    private WebSocketSession getSession(UUID workbookId, UUID userId) {
        Map<UUID, WebSocketSession> sessions = workbookSessions.get(workbookId);
