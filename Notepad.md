...
├──────────────────────────────────────────────────────────┤
│ - operationId: UUID                                      │
│ - type: OperationType (INSERT, DELETE, FORMAT)           │
│ - position: int                                          │
│ - content: string                                        │
│ - length: int                                            │
│ - userId: UUID                                           │
...
│                    OTEngine                              │
├──────────────────────────────────────────────────────────┤
│ - serverVersion: Map<documentId, int>                    │
│ - operationQueue: Map<documentId, Queue<Operation>>      │
├──────────────────────────────────────────────────────────┤
│ + transform(op1: Operation, op2: Operation): Operation   │
│ + applyOperation(doc: Document, op: Operation): void     │
│ + getServerVersion(docId: UUID): int                     │
│ + transformAgainstHistory(op, history): Operation        │
└──────────────────────────────────────────────────────────┘
...
├──────────────────────────────────────────────────────────┤
│ + handleOperation(ws: WebSocket, op: Operation): void    │
│ + broadcastOperation(docId: UUID, op: Operation): void   │
│ + addConnection(docId: UUID, ws: WebSocket): void        │
│ + removeConnection(docId: UUID, ws: WebSocket): void     │
│ + updateCursor(userId, position, selection): void        │
└──────────────────────────────────────────────────────────┘
...
├──────────────────────────────────────────────────────────┤
│ + createDocument(title, ownerId): Document               │
│ + getDocument(documentId): Document                      │
│ + updateDocument(documentId, content): void              │
│ + deleteDocument(documentId): void                       │
│ + shareDocument(docId, userId, accessLevel): void        │
└──────────────────────────────────────────────────────────┘
...
├──────────────────────────────────────────────────────────┤
│ + createSnapshot(document: Document): Version            │
│ + getVersionHistory(docId: UUID): List<Version>          │
│ + restoreVersion(docId: UUID, versionId: UUID): void     │
│ + pruneOldVersions(docId: UUID): void                    │
└──────────────────────────────────────────────────────────┘
...
│ + searchDocuments(query: string, userId): List<Result>   │
│ + updateIndex(docId: UUID): void                         │
│ + deleteFromIndex(docId: UUID): void                     │
└──────────────────────────────────────────────────────────┘
```
